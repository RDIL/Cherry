        ...
        if (MinecraftClient.isHudEnabled() && this.getEnabled()) {
            ...
            if (!collection.isEmpty()) {
                for (StatusEffectInstance statusEffectInstance : Ordering.natural().reverse().sortedCopy(collection)) {
                    ...
                    s.drawCenteredString(MinecraftClient.getInstance().textRenderer, I18n.translate(statusEffect.getTranslationKey()) + " - " + StatusEffectUtil.durationToString(statusEffectInstance, 1.0F), this.getConfigX(), this.getConfigY() + y, 16777215);
                    y += 10;
                }
            }
        }
